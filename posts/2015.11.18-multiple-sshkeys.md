...
The solution is a manual step and is a bit crap but at the same time, it takes not a lot longer than when I was retyping the host name and it actually works across everything (so far).

Simply have the following inside your ssh config (comment it out by default):
