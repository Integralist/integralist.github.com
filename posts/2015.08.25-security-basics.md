...
<!--language-bash-->
    ...
    gpg --default-key email@domain.com --sign secrets.txt

> Note: I added `--default-key <email>` because for me the default key wasn't the key I wanted to use for the signature

Once the file has been signed (it'll encrypt the file as well as if you'd used `-e` or `--encrypt`), then you can verify the signature using the following:
