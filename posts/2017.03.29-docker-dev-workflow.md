...
Finally, when the container is run we use a subshell to run multiple tasks at once (some people prefer having a dedicated run script for this, but whatever).
...
> † this means our compiled binary will be a linux based binary, so you can't run it on your host machine if it's not linux based (e.g. I'm using macOS). If you want to compiled for multiple OS's then consider using [Gox](https://github.com/mitchellh/gox)
...
Here is our dependency file's content, where we can see the [Logrus](https://github.com/sirupsen/logrus) dependency we've specified:
