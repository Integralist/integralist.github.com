...
Note: although you're looping over the collection multiple times, you are in fact looping over a slightly smaller collection each time
...
This ends up being `O(n₂)` in the worst case, but can be `O(n Log₂ n)`.
...
> Note: Quick Sort's performance depends on the pivot you choose (see the example code in the reference list below "Gist: Algorithms in Python")
