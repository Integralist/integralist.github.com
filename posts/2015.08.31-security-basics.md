...
- How to encrypt data using GPG
- GPG vs PGP
- OpenSSH vs OpenSSL
- Creating certificates and self-signing

### Plaintext and Ciphers

Throughout this post you'll see me use words like "plaintext" and "cipher". It's important to know what these means before moving on, so let's clarify this now:
...
In order for the SSL handshake to proceed successfully, the client needs to provide the server with some preliminary options; one being a "cipher suite". A cipher suite will looks something like the following:
...
What you might not be aware of though is how large a suite of cryptographic tools OpenSSL actually provides. Later on in this post we'll demonstrate a tiny selection of these tools in order to create our own keys, as well as encrypting data and creating certificates; but for now we'll focus more on the differences between OpenSSL and OpenSSH.
