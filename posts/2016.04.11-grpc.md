...
Now these are all things that can be worked around (or I could just build the Go service as a HTTP REST service and some of these problems dissapear). But it seems lots of people have been talking about using Google's new open-source RPC framework called [gRPC](http://www.grpc.io/).

Google define gRPC as:

> A high performance, open source, general RPC framework that puts mobile and HTTP/2 first

You can define and codify your service requirements via a `.proto` file, which is based on another concept they've been working on known as [Protocol Buffers](https://developers.google.com/protocol-buffers/docs/overview). Protocol buffers are an open source mechanism for serializing structured data.
