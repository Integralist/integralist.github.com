...
So how does that help us? Well, after the definition of the `HandlerFunc` func type, you'll also find the following snippet of code:
...
This associates the required `ServeHTTP` function with the `HandlerFunc` type. So if you convert your function to a `HandlerFunc` it will gain access to a `ServeHTTP` function! 
