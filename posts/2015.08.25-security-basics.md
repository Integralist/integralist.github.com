...
<!--language-bash-->
    ...
    gpg --export -a "Mark McDonnell" > gpg_public.key

To get the private key, you'll use a slightly different flag, but effectively it's the same thing:

<!--language-bash-->

    gpg --export-secret-key -a "Mark McDonnell" > gpg_private.key
...
<!--language-bash-->
    ...
    gpg --export -a my.email@gmail.com
...
You can also add a MAC (remember we looked at this earlier when discussing PKI; this is the digital signature) by adding the `--sign` flag:

<!--language-bash-->

    gpg --default-key email@domain.com --symmetric --sign secrets.txt

> Note: I added `--default-key <email>` because for me the default key wasn't the key I wanted to use for the signature

When you decrypt the gpg file (e.g. `gpg secrets.txt.gpg`) you'll now see (after you enter the relevant passphrase) the following signature output:
...
[Keybase](https://keybase.io/) was mentioned earlier when we were discussing the problem of 'authentication' and that there are many public repositories where you can locate a person's public key. [Keybase](https://keybase.io/) is a very recent attempt at trying to solve this problem in a modern way using modern technology.
...
Now if someone sends you an encrypted file using your Keybase public key, then you'd decrypt the file with:

<!--language-bash-->

    keybase pgp decrypt -i some_encrypted_file.gpg

If you want to encrypt a file for someone, then you would do:

<!--language-bash-->

    keybase encrypt -i secret_stuff.txt integralist

> Note: remember to swap `integralist` for whatever your Keybase username is
