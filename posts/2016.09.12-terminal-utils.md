...
> If you run tcpdump on a remote server and you want to use a gui like Wireshark to interrogate the recorded data, then you'll need to use `scp` to transfer the file back to your local machine

The output of the program may look a little confusing but there is consistent structure you can look out for:

<!--language-bash-->

    <date_time> <protocol> <src> > <dest>: Flags[<type>] <communication>
...
> Note: you'll see other information such as `win` in the communication output, this is a sliding 'window' that represents the size of the packets being sent back and forth. Typically an ACK won't be sent until the relevant packet size has been sent/received. Win is also used for flow control (one end is having trouble keeping up with the number of packets)

#### Last Packet?

It's important to realise that `FIN` wont necessarily be the last packet sent. 
